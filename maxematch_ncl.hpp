/* Maximal edge matching using MPI-3 Neighborhood Collectives */

#pragma once
#ifndef MAXEMATCHNCL_HPP
#define MAXEMATCHNCL_HPP
...
class MaxEdgeMatchNCL
{
    public:
        MaxEdgeMatchNCL(Graph* g): 
            g_(g), D_(0), M_(0), 
            sources_(0), targets_(0), 
            sendbuf_(0), recvbuf_(0),
            nghosts_in_target_(0),
            nghosts_target_indices_(0), pindex_(0),
            indegree_(-1), outdegree_(-1),             
            scounts_(0), rcounts_(0),
            sdispls_(0), rdispls_(0)
        {
            ...
            GraphElem disp = 0;
            nghosts_target_indices_.resize(outdegree_, 0);
        ...
        // print statistics about degrees in communication graph
        void print_nbr_stats()
        {
            int sumdeg = 0, maxdeg = 0;
            
            MPI_Reduce(&outdegree_, &sumdeg, 1, MPI_INT, MPI_SUM, 0, comm_);
            MPI_Reduce(&outdegree_, &maxdeg, 1, MPI_INT, MPI_MAX, 0, comm_);
            
            double my_sq = outdegree_*outdegree_;
            double sum_sq = 0.0;
            MPI_Reduce(&my_sq, &sum_sq, 1, MPI_DOUBLE, MPI_SUM, 0, comm_);
            ...
            if (rank_ == 0)
            {
                std::cout << std::endl;
                std::cout << "-------------------------------------------------------" << std::endl;
                std::cout << "Neighborhood graph topology characteristics" << std::endl;
                std::cout << "-------------------------------------------------------" << std::endl;
                std::cout << "Number of nodes (no. of processes): " << size_ << std::endl;
                std::cout << "Number of edges: " << (sumdeg / 2) << std::endl;
                std::cout << "Maximum out-degree: " << maxdeg << std::endl;
                std::cout << "Average out-degree: " << average << std::endl;
        ...
        void resize_alltoall_params()
        {
            scounts_.resize(outdegree_, 0);
            sdispls_.resize(outdegree_, 0);
            rcounts_.resize(indegree_, 0);
            rdispls_.resize(indegree_, 0);
        }

        void clear_alltoall_params()
        {
            scounts_.clear();
            sdispls_.clear();
            rcounts_.clear();
        ...
        void find_mate(GraphElem x)
        {
            ...
            if (y != -1)
            {
                ...
                else // send REQUEST
                {
                    ...
                    const GraphElem pidx = pindex_[y_owner];
                    const GraphElem index = nghosts_target_indices_[pidx] + scounts_[pidx];

                    sendbuf_[index] = y;
                    sendbuf_[index + 1] = x;
                    sendbuf_[index + 2] = MATE_REQUEST_TAG;
                    scounts_[pidx] += 3;
                }
            }
            else // mate[x] = -1, deactivate all x - adj(x) edges
            {
                ...
                for (GraphElem e = e0; e < e1; e++)
                {
                    ...
                    {   
                        ...
                        {
                            const GraphElem pidx = pindex_[z_owner];
                            const GraphElem index = nghosts_target_indices_[pidx] + scounts_[pidx];
                            nghosts_in_target_[pidx] -= 1;
                            
                            sendbuf_[index] = z;
                            sendbuf_[index + 1] = x;
                            sendbuf_[index + 2] = MATE_INVALID_TAG;
                            scounts_[pidx] += 3;
                        }
                    }
                }
                ...
                if (edge.active_)
                {
                    ...
                    if (mate_[g_->global_to_local(v)] != x)
                    {
                        ...
                        {                                                        
                            ...
                            const GraphElem pidx = pindex_[x_owner];
                            const GraphElem index = nghosts_target_indices_[pidx] + scounts_[pidx];
                            ...
                            sendbuf_[index] = x;
                            sendbuf_[index + 1] = v;
                            sendbuf_[index + 2] = MATE_REJECT_TAG;
                            scounts_[pidx] += 3;
                        }
                    }
                }
        ...
        // core neighborhood alltoall
        // --------------------------
        void nalltoall()
        { 
            // incoming data sizes
            // TODO FIXME scounts is used as a counter too, overflow 
            // problems imminent?
            MPI_Neighbor_alltoall(scounts_.data(), 1, MPI_INT, 
                    rcounts_.data(), 1, MPI_INT, g_comm_);

            // prepare recv buffers
            int rpos = 0;
            for (int i = 0; i < indegree_; i++)
            {
                sdispls_[i] = (int)nghosts_target_indices_[i];
                rdispls_[i] = rpos;
                rpos += rcounts_[i]; 
            }

            recvbuf_.resize(rpos);

            // incoming data
            MPI_Neighbor_alltoallv(sendbuf_, scounts_.data(), sdispls_.data(), 
                    MPI_GRAPH_TYPE, recvbuf_.data(), rcounts_.data(), rdispls_.data(), 
                    MPI_GRAPH_TYPE, g_comm_);   

            // flush counter
            memset(scounts_.data(), 0, sizeof(int)*outdegree_);
        }
                
        // remote operations, needs alltoall before progressing
        // ----------------------------------------------------        
        void process_messages()
        {
            ...
            for (int i = 0; i < recvbuf_.size(); i+=3)
            {
                g_l[0] = recvbuf_[i];
                g_l[1] = recvbuf_[i + 1];
                int status = recvbuf_[i + 2];
    ...
    private:
        ...
        std::unordered_map<int, GraphElem> 
            pindex_; // ghost vertices in target rank

        // intermediate communication buffers
        GraphElem* sendbuf_;
        std::vector<GraphElem> recvbuf_, 
            nghosts_target_indices_, nghosts_in_target_; // indices of data
        
        std::vector<int> sources_, targets_;
        std::vector<int> scounts_, rcounts_, sdispls_, rdispls_;
